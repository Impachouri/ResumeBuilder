...
import { EducationType, ExperienceType, ProjectType, SectionDataType, defaultState, link } from "./DefaultState";
...
export type SectionType = SectionDataType['personalInfo'] | 
...
export type SectionItemType = ExperienceType | ProjectType | EducationType | SectionDataType['personalInfo']


export const SectionDataReducers: Reducer<SectionDataType, SectionDataAction> = (
  state,
  action
) => {
...
let updatedSection: SectionType;
  ...
  switch (action.type) {
    ...
    case "LINK_INPUT":
      ...
      updatedSection = Array.isArray(section)
        ...
        : {
            ...
            links: (section as { links: link[] }).links.map((link: link, index: number) =>
    ...
    case "REMOVE_LINK":
      ...
      updatedSection = Array.isArray(section)
        ? (section.map((item:SectionItemType, index:number)=>
          ...
          ? {
            ...item,
            links: item.links.filter((_link:link, linkIndex: number)=>
        ...
        : {
          ...(section as {links: link[]}),
          links: (section as {links: link[]}).links.filter((_link:link, linkIndex: number)=>
            linkIndex !== action.data.index
          )
        } as SectionType
    ...
    case "REMOVE_EXPERIENCE":
      section = state.experience.filter((_exp: ExperienceType, index: number)=>
        index !== action.data.index
      )
    ...
    case "REMOVE_PROJECT":
      section = state.projects.filter((_pro: ProjectType, index: number)=>
        index !== action.data.index
      )
    ...
    case "REMOVE_EDUCATION":
      section = state.education.filter((_edu: EducationType, index: number)=>
        index !== action.data.index
      )
...
export const useSectionReducers = () => {
    ...
    return {sectionState, dispatch} as  { sectionState: SectionDataType; dispatch: React.Dispatch<SectionDataAction> };
}
