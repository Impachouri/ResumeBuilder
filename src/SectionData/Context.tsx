...
export const SectionDataProvider: FC<SectionDataProviderProps> = ({ children }) => {
  const {sectionState, dispatch} = useSectionReducers();
  const [activeSection, setActiveSection] = useState<keyof SectionDataType>('personalInfo');

  return (
    <SectionDataContext.Provider value={{sectionState, dispatch, activeSection, setActiveSection}}>
      {children}
